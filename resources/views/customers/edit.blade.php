...
@section('content')
    <div class="max-w-2xl mx-auto bg-white p-6 rounded shadow">
        <h1 class="text-2xl font-bold mb-4">Actualizar clientes</h1>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="document_type" id="document_type" class="w-full px-4 py-2 border rounded">
                        <option value="INE" {{ $customer->document_type == "INE" ? 'selected' : '' }}>INE</option>
                        <option value="RFC" {{ $customer->document_type == "RFC" ? 'selected' : '' }}>RFC</option>
                        <option value="PASAPORTE" {{ $customer->document_type == "PASAPORTE" ? 'selected' : '' }}>PASAPORTE</option>
                        <option value="Cedula" {{ $customer->document_type == "Cedula" ? 'selected' : '' }}>Cedula</option>
                    </select>
                </div>
                <div>
